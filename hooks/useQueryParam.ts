...
/**
 * 단일 쿼리 파라미터의 값을 읽어와 반환하는 Getter 함수입니다.
 * 쿼리 파라미터의 값이 1개 이상 존재하거나 fallback 값이 주어진다면 한 개의 값이 보장되는 string[] 타입, 아니라면 undefined를 반환합니다.
 *
...
>(name: TKey, fallback?: TFallback) {
  ...
  const nonEmptyArrayOrUndefined =
    values.length > 0 ? values : fallback ? [fallback] : undefined;

  return nonEmptyArrayOrUndefined as TFallback extends TValue
    ? TValue[]
    : TValue[] | undefined;
}
